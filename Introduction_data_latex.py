...
for i in range(10):
    ...
    print(f"Instance: {i+1}, Seed: {seeds[i]}, mu: {mu:.2f}, sigma: {sigma:.2f}, coefficient of variation: {v:.2f}")
    print(r"\begin{table}[ht]")
    print(r"\centering")
    print(r"\begin{tabular}{c c c c c c c c}")
    print(r"\hline")
    print(r"Supplier & $p_i$ & $y_i$ & $c_i$ & $c_i^{\mathrm{eff}}$ & $CR_i$ & $E[R_i]$ & $\operatorname{Var}(R_i)$ \\")
    print(r"\hline")
    for row in New_table:
        formats = ["{:.0f}", "{:.2f}", "{:.2f}", "{:.1f}", "{:.1f}", "{:.2f}", "{:.2f}", "{:.2f}"]
        print(" & ".join(format_spec.format(value) for format_spec, value in zip(formats, row)) + r" \\")
    print(r"\hline")
    print(r"\end{tabular}")
    print(r"\end{table}")
